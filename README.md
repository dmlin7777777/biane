...
| **效果展示** | 文字描述功能（"可以生成 PPT"） | 按类型选择：截图 / Before/After 对比 / 模式清单 / 生成文本 |
...
用这个 Skill 的 Type 2（提效型）策略重写了 [baogong-skill](https://github.com/LearnPrompt/baogong-skill) 的 README。以下是关键改动对比：
...
> **核心差异**：原 README 很强，但它假设读者有耐心读完全文。Type 2 策略做的是：让读者在 3 秒内看到"凭什么装"的答案——不靠读完，靠扫读。
...
启动后，Skill README Writer 走三步：

| 步骤 | 做什么 | 输出 |
|------|--------|------|
| **1. 类型识别** | 根据 Skill 的功能判断属于 8 种类型中的哪一种 | 类型标签 + 对应策略 |
| **2. 策略分发** | 加载该类型的必选内容清单 + 标杆参考 | 结构化 README 提纲 |
| **3. 写作执行** | 按铁律 + 反模式检查 + 标杆对标来写 | 完整的 README.md |
...
| 标杆案例 | baogong / luban / nuwa / guizang / cailun / paoding / humanizer / trailofbits / K-Dense-AI |
| 文档规模 | SKILL.md 550 行 + test-prompts.json + examples/ + references/research-data.md |
| 跨类型速查表 | 8 类型 × 7 必选项 = 56 格完整覆盖 |
| 常见错误 | 5 个高频反模式 + 修正方案 |
| 失败模式 | 3 种失败场景 + fallback 流程 |
